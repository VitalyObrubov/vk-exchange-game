...
class GameAccessor(BaseAccessor):
    ...
    async def start_game(self, chat_id: int):
        raw_users = await self.app.store.vk_api.get_users(chat_id)
        game = Game(id=0, 
                    create_at=datetime.now(),
                    chat_id=chat_id,
                    state="started",
                    ...
                    users=[],
                    traded_sequrites=[])
        self.create_game_users(game, raw_users)
        ...
        if res == None:
            ...
            text = """Игра начата!
                      У каждого на счету 10000 монет
                      Продаются следующие акции      
            """
            for security in game.traded_sequrites:
                text += f"{security.id} {security.description} за {security.price} монет {security.market_event}\n"
            return text
        else:
            return res
